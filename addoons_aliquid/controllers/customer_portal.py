...
class CustomerPortalInvoice(invoiceportal):
    ...
    def portal_my_invoices(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        searchbar_sortings = {
            'date': {'label': 'Invoice Date', 'order': 'invoice_date desc'},
            'duedate': {'label': 'Due Date', 'order': 'invoice_due_date desc'},
        ...
        if request.env.user.portal_invoice_access == 'own':
            invoices = request.env['account.move'].sudo().search(self.get_my_domain(partner, 'invoice'))
            invoice_id_list = []
            ...
            invoice_ids = request.env['account.move'].sudo().browse(invoice_id_list)
...
class CustomerPortalInherit(CustomerPortal):
    ...
    def my_prepare_values(self):
        ...
        if request.env.user.portal_invoice_access == 'all':
            values['invoice_count'] = request.env['account.move'].sudo().search_count(self.get_my_domain(partner, 'invoice'))
        else:
            invoices = request.env['account.move'].sudo().search(self.get_my_domain(partner, 'invoice'))
            invoice_id_list = []
            for invoice in invoices:
